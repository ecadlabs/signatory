...
main() {
  ...
  while IFS= read -r protocol; do
    ...
    protocol_name=$(echo "$protocol" | sed 's/^[0-9]\{3\}-//')
    ...
    local_baker="$tezos_bin_dir/octez-baker-$protocol_name"
    local_accuser="$tezos_bin_dir/tezos-accuser-$protocol_name"
    parameters_file="$tezos_script_dir/$protocol-parameters/sandbox-parameters.json"
    hash=$(grep "^$protocol_name" $bin_dir/protocol_hash)
...
alias octez-autocomplete="if [ \$ZSH_NAME ] ; then autoload bashcompinit ; bashcompinit ; fi ; source \"$tezos_script_dir/bash-completion.sh\"" ;
