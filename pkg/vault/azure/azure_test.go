...
func serverhandler() (http.Handler, error) {
	...
	uri := managementURL + `/subscriptions/{SubscriptionID}
	...
	r.Methods("POST").Path("/keys/{key}").HandlerFunc(getKey)
	r.Methods("GET").Path("/.default/keys/{key}").HandlerFunc(sign)
	r.Methods("PUT").Path("/.default/keys/{key}").HandlerFunc(importPkey)
	r.Methods("GET").Path(uri).HandlerFunc(mgmtRequest)
...
func getVault(ctx context.Context) (*ahsm.Vault, error) {
	...
	return ahsm.New(ctx, &ahsm.Config{
		...
		VaultScopeURL:  []string{`http://localhost:8080/.default`},
		ManagementURL:  []string{`http://localhost:8080/.default`},
	})
}
