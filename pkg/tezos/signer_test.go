...
func TestImport(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []*tezos.PublicKey{imported}, list)
}
...
func TestPolicy(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.title, func(t *testing.T) {
			...
			_, err = s.Sign(context.Background(), &tezos.SignRequest{PublicKeyHash: pk.Hash(), Message: c.msg})
			if c.expected != "" {
